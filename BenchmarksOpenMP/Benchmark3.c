...
int main(int argc, char *argv[])
{
    ...
    //Se define la matriz transpuesta
    int **matriztrans;
    matriztrans = (int **)malloc(valor * sizeof(int *));

    for (int i = 0; i < valor; i++)
        matriztrans[i] = (int *)malloc(valor * sizeof(int));

    //Se genera la matriz transpuesta de lamatriz2
    for (int i = 0; i < valor; i++)
    {
        for (int j = 0; j < valor; j++)
        {
            matriztrans[i][j] = matriz2[j][i];
        }
    }
...
#pragma omp parallel
    for (int i = 0; i < valor; i++)
    {
        for (int j = 0; j < valor; j++)
        {
            ...
            {
                //Guarda el resultado de la multiplicacion entre matriz y matriztrans
                matrizmult[i][j] += matriz[i][k] * matriztrans[j][k];
            }
        }
    }
